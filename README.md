...
当前稳定版本为 **v1.0.0**。转换核心与第一版交互已经冻结，详细变更见 [CHANGELOG.md](CHANGELOG.md)。当前不制作安装包；后续格式范围基本稳定后，再统一评估 Windows 等平台的打包方式。
...
测试包括编码识别、TXT 规则、AST 统计、路径安全、同名避让、跳过状态、五文件批处理、ZIP 内容、本地图片嵌入与提取，以及在 Pandoc 可用时执行六条真实转换路径。
...
更详细的模块边界、安全策略和取舍见 [设计文档.md](设计文档.md)。
